...
1.添加一个对于C的相对提百分比提升显示,但不知道如何计算C的伤害提升,也缺少数据模板,无限期搁置<br>
2.类似光环宠物等1~95技能等级加+1,对于此类加等级的装备如果手动计算非常麻烦.思路:增加一个弹出面板,计算出增加的四维,然后再加到计算器里去.需要考虑到技能的成长曲线包括(是否线性成长，是否有上限),四个奶加起来的工程量极大,无限期搁置.<br>
3.Bug-1.输入0开头的数计算会直接退出.<br>
<h2 id="-">三.示例</h2>
